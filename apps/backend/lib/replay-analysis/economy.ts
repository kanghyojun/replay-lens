...
import { gameLoopToSeconds, formatTime, THIRTY_SECONDS_IN_LOOPS } from './utils';
import type { EconomySnapshot, EconomyTimeline } from './types';
...
export function extractEconomy(
  ...
  const playerEvents: Record<number, Array<{ gameloop: number; stats: Record<string, number> }>> = {};
